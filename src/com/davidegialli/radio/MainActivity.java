...
public class MainActivity extends BaseActivity {
	...
	//for background notification update
	onPauseReceiver onPauseReceiver;
	...
	protected void onPause() {
		...
		if (radioUpdateReceiver != null) {
			...
			onPauseReceiver = new onPauseReceiver();

			registerReceiver(onPauseReceiver, new IntentFilter(RadioService.MODE_METADATA_UPDATED));
			updateStatus(12);
		}
		// finish();
	}
	...
	protected void onResume() {
		...
		if (radioService != null) {
			if (isStationChanged) {
				...
				if (radioService.isPlaying()) { //Makes sure the correct buttons are enabled when returning to activity
	...
	private class onPauseReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			if (intent.getAction().equals(RadioService.MODE_METADATA_UPDATED)) {
				...
				if (radioService.getTrack() != "" && radioService.getArtist() != "" 
						&& radioService.getTrack() != null && radioService.getArtist() != null) {

					radioService.updateNotification(radioService.getTrack() + " - " + radioService.getArtist());
				} else {

					radioService.updateNotification(radioService.getCurrentStationName());
				}
			}
		}
	...
	public void updateStatus(int intent) {
		String status = radioService.getStatus();
		if (radioService.getTotalStationNumber() > 1) {

			if (intent == 0) {

				statusTextView.setText(RadioService.MODE_DESTROYED);
			} else if (intent == 1) {

				statusTextView.setText(RadioService.MODE_COMPLETED);
			} else if (intent == 2) {

				statusTextView.setText(RadioService.MODE_CONNECTING);
			} else if (intent == 3) {

				statusTextView.setText(RadioService.MODE_START_PREPARING);
			} else if (intent == 4) {

				statusTextView.setText(RadioService.MODE_PREPARED);
			} else if (intent == 5) {

				statusTextView.setText(RadioService.MODE_BUFFERING_START);
			} else if (intent == 6) {

				statusTextView.setText(RadioService.MODE_BUFFERING_END);
			} else if (intent == 7) {

				statusTextView.setText(RadioService.MODE_PLAYING);
			} else if (intent == 8) {

				statusTextView.setText(RadioService.MODE_PAUSED);
			} else if (intent == 9) {

				statusTextView.setText(RadioService.MODE_STOPPED);
			} else if (intent == 10) {

				statusTextView.setText(RadioService.MODE_COMPLETED);
			} else if (intent == 11) {

				statusTextView.setText(RadioService.MODE_ERROR);
			} else if (intent == 12) {

				statusTextView.setText(RadioService.MODE_METADATA_UPDATED);
			}
