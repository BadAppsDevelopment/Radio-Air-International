...
OnCompletionListener, OnPreparedListener, OnInfoListener {
	...
	private static int AAC_BUFFER_CAPACITY_MS = 7500; //was at 5000
	private static int AAC_DECODER_CAPACITY_MS = 700; //was at 700
	...
	public void onCreate() {
		...
		try {
			...
			System.out
			.println("Cannot set the ICY URLStreamHandler - maybe already set ? - " 
					+ t);
		}
		...
		try {
			...
			multiPlayer = new MultiPlayer(multiPlayerCallback, AAC_DECODER_CAPACITY_MS, AAC_DECODER_CAPACITY_MS);
	...
	public void play() {
		if (hasConnectivity()) {
			if (getCurrentStationType().equals(TYPE_AAC)) {
				...
				if (isPrepared) { //make sure mediaPlayer isn't null
					isRadioPlaying = true;
					mediaPlayer.start();
		...
		try {
			...
			if (notifyBuilder != null && notifyMgr != null) {
				...
				if (track == "" && artist == "") {
					...
					notifyBuilder.setContentText(getCurrentStationName());
				} else if (track != " " && artist != " " && track != null && artist != null){
					//Check to make sure artist and track string values aren't empty or null
					notifyBuilder.setContentText(track + " - " + artist);
				} else {
				...
				notifyBuilder.setContentText(status);
			}	
				...
				handler.post(new Runnable() {
					...
					public void run() {
						try {
							...
							Log.i("startRefreshingMetadata","error is in startRefreshingMetadata function");
							// e.getMessage());
						}
					}
				});
